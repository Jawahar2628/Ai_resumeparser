...
import { Mail, Save, Server, Key, User, ToggleLeft, ToggleRight, CheckCircle2, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-400 mt-2">Configure system settings and email preferences</p>
      </div>
      ...
      <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-xl">
        <div className="flex items-center gap-3 mb-6 border-b border-slate-800 pb-4">
          ...
          <h2 className="text-xl font-semibold text-slate-200">Email Configuration (SMTP)</h2>
        </div>

        {message.text && (
          <div className={`p-4 rounded-lg mb-6 flex items-center gap-3 ${message.type === 'success' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border border-rose-500/20 text-rose-400'}`}>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg py-2.5 pl-10 pr-4 text-slate-200 focus:outline-none focus:border-indigo-500 transition-colors"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">
                Password {config.smtp_password_set && <span className="text-emerald-400 text-xs ml-2">(Saved)</span>}
              </label>
              <div className="relative">
          ...
          <div className="flex items-center gap-8 py-4 border-t border-slate-800/50">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => setConfig({...config, use_tls: !config.use_tls})}>
              {config.use_tls ? <ToggleRight className="w-8 h-8 text-indigo-400" /> : <ToggleLeft className="w-8 h-8 text-slate-600" />}
            ...
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => setConfig({...config, use_ssl: !config.use_ssl})}>
              {config.use_ssl ? <ToggleRight className="w-8 h-8 text-indigo-400" /> : <ToggleLeft className="w-8 h-8 text-slate-600" />}
              <span className="text-slate-300">Use SSL</span>
            </div>
          </div>
      ...
      <div className="mt-8 bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-xl">
        <div className="flex items-center gap-3 mb-6 border-b border-slate-800 pb-4">
          <div className="p-2 bg-cyan-500/20 rounded-lg">
            <Mail className="w-6 h-6 text-cyan-400" />
          </div>
          <h2 className="text-xl font-semibold text-slate-200">Test Configuration</h2>
        </div>
        <div className="flex items-end gap-4">
          <div className="flex-1">
             <label className="block text-sm font-medium text-slate-300 mb-2">Test Email Address</label>
             <input
                type="email"
                value={testEmail}
                onChange={e => setTestEmail(e.target.value)}
                className="w-full bg-slate-800 border border-slate-700 rounded-lg py-2.5 px-4 text-slate-200 focus:outline-none focus:border-cyan-500 transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 px-6 py-2.5 rounded-lg font-medium transition-colors disabled:opacity-50"
          >
